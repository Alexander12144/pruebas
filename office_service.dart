  ...
  Future getOffices() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        debugPrint("Descargando Oficinas...");
        await SqlDb().deleteData("DELETE FROM Oficinas");
        String date = getDateTimeSync();
        for (var item in decryptedResponseJson) {
          var sql =
              ("INSERT INTO Oficinas (idOficina,oficina,idRegion,ambito,fechaSync) VALUES (${item['IDOFICINA']},'${item['OFICINA']}',${item['IDREGION']},0,'$date')");
          await SqlDb().insertData(sql);
        }
      } else {
        debugPrint("Error: OfficeService -> ${response.statusCode}");
      }
    } catch (e) {
      debugPrint("Error: OfficeService -> $e");
    }
  }
}

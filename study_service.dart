...
class StudyService {
  Future getAllStudy() async {
    Uri url = getURL('load_nivel_estudio');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        debugPrint("Descargando Nivel de Estudio...");
        await SqlDb().deleteData("DELETE FROM NivelEstudio");
        String fecha = DateTime.now().toString();
        for (var item in decryptedResponseJson) {
          var sql = ("""
            INSERT INTO NivelEstudio (
              codNivelEstudio,
              nivelEstudio,
              fechaSync
            )
            VALUES (
              '${item['CODNIVELESTUDIO']}',
              '${item['NIVELESTUDIO']}',
              '$fecha'
            )
          """);
        ...
        debugPrint("Error: StudyService -> ${response.statusCode}");
      }
    } catch (e) {
      debugPrint("Error: StudyService -> $e");
    }
  }
}

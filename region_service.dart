  ...
  Future getRegions() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        debugPrint("Descargando Regiones...");
        await SqlDb().deleteData("DELETE FROM Regiones");
        String date = getDateTimeSync();
        for (var item in decryptedResponseJson) {
          var sql =
              ("INSERT INTO Regiones (idRegion,region,ambito,fechaSync) VALUES (${item['IDREGION']},'${item['REGION']}',0,'$date')");
          await SqlDb().insertData(sql);
        }
      } else {
        debugPrint("Error: RegionService -> ${response.statusCode}");
      }
    } catch (e) {
      debugPrint("Error: RegionService -> $e");
    }
  }
}
